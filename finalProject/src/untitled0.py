...
X_trainR, X_testR, y_trainR, y_testR = train_test_split(X_reference, y, test_size = 0.0, random_state = 0)
...
X_2019 = preprocess_input(prospects_2019_data)


pred_WS_2019 = model.predict(X_2019)
pred_WS_2019 = y_scaler.inverse_transform(pred_WS_2019).reshape(len(pred_WS_2019))



df = pd.DataFrame()
df["prospects"] = list(X_2019_reference.playerName)
df["predicted peak in NBA win shares"] = list(pred_WS_2019)


df = df.sort_values(by="predicted peak in NBA win shares", ascending=False)

x = df["predicted peak in NBA win shares"][0:15]
y = df["prospects"][0:15]



df["prospects"] = y
df["predicted peak in NBA win shares"] = x
...
ax = sns.barplot(x="predicted peak in NBA win shares", y="prospects",data=df)
print(good_names)
